...
class _EditPersonPageState extends State<EditPersonPage> {
  ...
  void initState() {
    ...
    _designation =
        p.designation.isNotEmpty && kDesignations.contains(p.designation)
        ? p.designation
    ...
    _selectedDepot = p.depot.isNotEmpty
        ? p.depot
        : (kBranchDepots[_selectedBranch]?.first ?? '');
    if (_selectedBranch == 'Other') {
      // Allow custom depot editing
      _showOtherDepotField =
          (_selectedDepot?.isNotEmpty ?? false) &&
          _selectedDepot != 'Head Office';
      _otherDepotController.text = _selectedDepot ?? '';
    }
    final initialAccess =
        kDesignationAccessLevels[_designation] ?? p.accessLevel;
  ...
  List<Map<String, String>> _changes(Person p) {
    ...
    if (_first.text.trim() != p.firstName) {
      c.add({
        'field': 'First Name',
        'old': p.firstName,
        'new': _first.text.trim(),
      });
    }
    if (_last.text.trim() != p.lastName) {
      c.add({
        'field': 'Last Name',
        'old': p.lastName,
        'new': _last.text.trim(),
    ...
    if ((_selectedDepot ?? '') != p.depot) {
      c.add({'field': 'Depot', 'old': p.depot, 'new': _selectedDepot ?? ''});
    }
    if (_emp.text.trim() != p.employeeNo) {
      c.add({
        'field': 'Employee No',
        'old': p.employeeNo,
        'new': _emp.text.trim(),
    ...
    if (_accessLevel.text.trim() != p.accessLevel) {
      c.add({
        'field': 'Access Level',
        'old': p.accessLevel,
        'new': _accessLevel.text.trim(),
      });
    }
    return c;
  }
                          ...
                          return Column(
                            ...
                            children: [
                              DropdownButtonFormField<String>(
                                ...
                                initialValue:
                                    _selectedDepot != null &&
                                        depots.contains(_selectedDepot)
                                    ? _selectedDepot
                                    : null,
                                decoration: const InputDecoration(
                                ...
                                onChanged: (value) {
                                  if (value == 'Other') {
                                    setState(() {
                                      ...
                                      _selectedDepot = _otherDepotController
                                          .text
                                          .trim();
                                    });
                                  } else if (value != null) {
                  ...
                  children: [
                    Expanded(
                      child: DropdownButtonFormField<String>(
                        initialValue: _designation.isNotEmpty
                            ? _designation
                            : null,
                        items: kDesignations
                            .where((e) => e != 'All')
