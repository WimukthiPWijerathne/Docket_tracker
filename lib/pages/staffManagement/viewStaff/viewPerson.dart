...
class _ViewPeoplePageState extends State<ViewPeoplePage> {
  ...
  bool _matchesSearchQuery(Person p, String query) {
    ...
    return searchableFields.any((field) => field.toLowerCase().contains(query));
  }
  ...
  void _navigateToPersonDetail(Person person) {
    Navigator.of(context)
        .push(
          MaterialPageRoute(builder: (_) => PersonDetailPage(person: person)),
        )
        .then((_) {
  ...
  List<Map<String, String>> _generateChangesList(
    Person p,
    String firstName,
    String lastName,
    String depot,
    ...
    if (depot != p.depot) {
      changes.add({'field': 'Depot', 'old': p.depot, 'new': depot});
    }
                  ...
                  children: [
                      ...
                      children: [
                        Expanded(
                          child: DropdownButtonFormField<String>(
                            initialValue: selectedDepot.isNotEmpty
                                ? selectedDepot
                                : null,
                            items: kDepots
                                .where((e) => e != 'All')
                    ...
                    Row(
                      children: [
                        Expanded(
                          child: DropdownButtonFormField<String>(
                            initialValue: designation.isNotEmpty
                                ? designation
                                : null,
                            items: kDesignations
                                .where((e) => e != 'All')
                          ...
                          child: ElevatedButton(
                            onPressed: () async {
                              if (formKey.currentState?.validate() ?? false) {
                                ...
                                final changes = _generateChangesList(
                                  p,
                                  first.text,
                                  last.text,
                                  selectedDepot,
                                ...
                                if (confirm == true) {
                                  final result = await _svc.updatePerson(
                                    ...
                                    fields: {
                                      ...
                                      'accessLevel': accessLevelController.text
                                          .trim(),
                                    },
                                  );
                                  if (!mounted) return;
  ...
  Widget _personCard(Person p) {
    ...
    final meta4 =
        'Access: ${p.accessLevel.isNotEmpty ? p.accessLevel : 'No Access'}';

    // Determine if we need to show compact layout
    final showCompactLayout = MediaQuery.of(context).size.width < 400;

    final activeColor = p.isActive
        ? Colors.green.shade700
        : Colors.red.shade700;
    final activeBg = p.isActive ? Colors.green.shade100 : Colors.red.shade100;
            ...
            builder: (ctx, c) {
              ...
              return Row(
                ...
                children: [
                  ...
                  ConstrainedBox(
                    constraints: BoxConstraints(maxWidth: isMobile ? 150 : 240),
                    child: actions,
                  ),
                ],
              );
                        ...
                        children: [
                          ...
                          RefreshIndicator(
                            ...
                            child: ListView.builder(
                              ...
                              itemBuilder: (_, i) => _personCard(activeList[i]),
                            ),
                          ),
                          // Inactive
                          RefreshIndicator(
